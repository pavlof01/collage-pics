  ...
  constructor(props) {
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        const {
          panningX, panningY, width, height, initialWidth, initialHeight, ratio,
        } = this.state;
        ...
        const { moveX } = gestureState; // MOVEMENT ALONG THE X
        const { moveY } = gestureState; // MOVEMENT ALONG THE Y
        ...
        if (gestureState.numberActiveTouches == 1) {
          ...
          if (!this.scaling) {
            ...
            if (
              (newWidth > relativeContainerWidth && height > relativeContainerHeight)
              || incrementScaling < 0
            ) {
